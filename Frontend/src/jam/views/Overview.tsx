...
function JamHosts(jamData: jamData) {
  return (
    <div>
      Hosted by:&nbsp;
...
function JamDate(jam: jam) {
  ...
  return (
    <div>
      This Jam ran
      from <strong>{format(parseISO(jam.started), 'dd.MM.yyyy')} to {format(parseISO(jam.ended), 'dd.MM.yyyy')},</strong>
      <br/>
...
function JamStats(jam: jam) {
  return (
    <div>
      <strong>{jam.ratings}</strong> Ratings<br/>
      <strong>{jam.entries}</strong> Entries
    </div>
  );
}

function Overview({jamData}: { jamData: jamData }) {
  return (
    <div className="view" id="Overview" style={{backgroundColor: `${jamData.jam.bg_color}`}}>
      <h1 className="jam-title">
        <a href={jamData.jam.url} target="_blank" rel="noopener noreferrer"
           className="jam-title-link">{jamData.jam.Title} </a>
      </h1>
      ...
      <div className="card-grid">
        ...
        />
        <JsxCard
          jsx={<a href={jamData.jam.twitter.twitter_link} target="_blank"
                  rel="noopener noreferrer">{jamData.jam.twitter.hashtag}</a>}
